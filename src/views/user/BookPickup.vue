<template>
  <div class="p-4 sm:p-6 lg:p-10 min-h-screen">
    ...
    >
      ...
      >
        <path
          d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172l-4.293-4.293a2 2 0 00-2.828 2.828l6 6a2 2 0 002.828 0l10-10a2 2 0 000-2.828zM10 18a8 8 0 100-16 8 8 0 000 16zM10 4a6 6 0 110 12 6 6 0 010-12z"
          clip-rule="evenodd"
          fill-rule="evenodd"
        ></path>
      </svg>
    ...
    <GradientLoader v-if="isLoadingServices">
      Fetching services and prices...
    </GradientLoader>
    ...
    >
      <p class="font-semibold mb-2">
        check your internet connection {{ servicesError }}
      </p>
      <button
        @click="fetchServices"
    ...
    <div v-else class="grid lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="p-6 rounded-xl border border-golden-brown/30 space-y-4">
          <h2 class="text-xl font-semibold text-navy-blue border-b pb-2 mb-4">
            Order Configuration
          </h2>
          <div class="grid sm:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div>
              ...
              <p
                ...
                class="text-xs text-charcoal/70 mt-1"
              >
        ...
        <p
          ...
          class="text-charcoal font-semibold p-4 bg-red-900/10 rounded-lg border-l-4 border-destructive"
        >
        ...
        <h2
          class="text-xl font-semibold text-charcoal border-b border-input pb-2 pt-4"
        >
          Services ({{ orderPayload.pricingModel }} -
          {{ orderPayload.serviceTier }})
        </h2>

        <div
          v-if="groupedServices.length === 0"
          class="text-center py-10 text-charcoal/70"
        >
        ...
        <div
          ...
          :class="{
            ...
            'bg-bone-white border border-golden-brown/30': !isItemSelected(
              service.code
            ),
        ...
        >
          ...
          <div
            class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-3"
          >
            <!-- Details -->
            ...
            <div class="flex items-center space-x-3">
              ...
              <button
                @click="
                  removeItem(service.code, selectedAddOns[service.code] || [])
                "
              ...
              <span
                class="w-8 text-center font-extrabold text-xl text-navy-blue transition-colors duration-300"
              >
          ...
          <!-- Add-ons Section -->
          <div
            v-if="service.addOns && service.addOns.length"
            class="pt-3 mt-3 border-t border-input/50"
          >
            <p class="text-sm font-semibold text-charcoal/90 mb-2">
              Available Add-ons:
            </p>
      ...
      <div class="lg:col-span-1">
        ...
        <div
          class="sticky lg:top-10 p-6 rounded-xl bg-charcoal border border-golden-brown/30 text-bone-white"
        >
          <h2
          ...
          <div
            v-if="orderPayload.items.length === 0"
            class="text-center py-6 text-cream/70"
          >
          ...
          <div v-else class="space-y-4">
            ...
            >
              ...
              <ul
                ...
                class="text-xs text-cream/70 ml-2 mt-1 space-y-0.5"
              >
                <li
                ...
                >
                  ...
                  <span class="font-medium text-pure-gold/80"
                    >₦{{ addon.price.toLocaleString() }}</span
                  >
                </li>
              </ul>
          ...
          <p
            v-if="orderPayload.items.length > 0"
            class="mt-4 text-xs text-cream/60 text-center"
          >
            * Final price (including discounts and delivery fee) is calculated
            next.
          </p>
        </div>
      </div>
...
const toggleAddOn = (serviceCode, addon) => {
  ...
  if (existingIndex >= 0) {
    ...
    if (addonNameLower.includes("express")) {
      ...
      currentAddOns = currentAddOns.filter(
        (a) => !a.name.toLowerCase().includes("same-day")
      );
    } else if (addonNameLower.includes("same-day")) {
      // Remove express if same-day is selected
      currentAddOns = currentAddOns.filter(
        (a) => !a.name.toLowerCase().includes("express")
      );
    }
